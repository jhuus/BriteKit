...
## Ensembling
Combining multiple checkpoints in an ensemble is a quick and easy way to improve classifier results. This can be especially powerful when different model architectures is used, but even with the same model type and training protocol, ensembling almost always improves results.
...
Given a per-segment test and a directory containing checkpoints, use the `ensemble` command to find the highest-scoring ensemble of a given size.
