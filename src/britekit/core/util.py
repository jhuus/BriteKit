...
def labels_to_list(
    input_path: str, recording_classes: Optional[dict[str, set[str]]] = None
    ...
    try:
        ...
        for label_path in label_paths:
            ...
            if recording_classes is not None and file_prefix not in recording_classes:
                continue
            ...
            for line in lines:
                ...
                try:
                    if is_birdnet:
                        ...
                        name = tokens2[0]
                        if (
                            recording_classes is not None
                            and name not in recording_classes[file_prefix]
                        ):
                            continue
...
def labels_to_dataframe(
    ...
    labels = labels_to_list(input_path, recording_classes)
...
def inference_output_to_dataframe(
    input_path: str, recording_classes: Optional[dict[str, set[str]]] = None
):
    """
        ...
        if len(csv_files) > 1:
            for csv_file in csv_files:
                ...
                if stem == "scores" or stem.endswith("_labels"):
                    ...
                    return _filter_inference_dataframe(df, recording_classes)
            ...
            df = labels_to_dataframe(input_path, recording_classes)
            return df
    except Exception as e:
...
def _filter_inference_dataframe(df, recording_classes):
    """Restrict inference output to selected (recording, class) pairs."""
    if recording_classes is None or df.empty:
        return df

    import pandas as pd

    requested_pairs = pd.MultiIndex.from_tuples(
        (recording, name)
        for recording, names in recording_classes.items()
        for name in names
    )
    row_pairs = pd.MultiIndex.from_arrays([df["recording"], df["name"]])
    return df[row_pairs.isin(requested_pairs)]
