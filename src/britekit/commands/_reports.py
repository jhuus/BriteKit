...
def rpt_epochs(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for epoch_num in epoch_nums:
            ...
            util.set_logging()
            logging.info(
                f"Epoch {epoch_num}: micro-averaged PR-AUC = {pr_score:.3f}, "
                f"micro-averaged ROC-AUC = {roc_score:.3f}"
            )
...
@click.option(
    ...
    default=True,
    help="Write intermediate y_true/y_pred matrix CSVs for segment tests (default: enabled).",
)
def _rpt_test_cmd(
    cfg_path: str,
    granularity: str,
